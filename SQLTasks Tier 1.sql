...
SELECT DISTINCT CONCAT(subquery.firstname, ' ', subquery.surname) AS name 
...
SELECT CONCAT(subquery.firstname, ' ', subquery.surname) AS name, subquery.facility, subquery.cost
FROM( SELECT firstname, surname, Facilities.name as facility, Bookings.starttime AS date,
     (CASE 
